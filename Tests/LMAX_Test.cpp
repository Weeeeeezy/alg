...
{
  //=========================================================================//
  // "TestStrategy_LMAX" Class:                                              //
  //=========================================================================//
  class TestStrategy_LMAX final: public Strategy
  {
  private:
    //=======================================================================//
    // Data Flds and Ctor:                                                   //
  ...
  public:
    ...
    override
    {
      ...
      if (!a_on && !m_exiting)
      {
        ...
        m_exiting = true;
        m_reactor->ExitImmediately("LMAX_Test::InTradingEvent");
      }
  ...
  public:
    ...
    override
    {
      ...
      //      {
      //        m_exiting = true;
      //        m_reactor->ExitImmediately("LMAX_Test::OnOrderBookUpdate: Error");
      //      }
      long lat_usec = (a_ts_strat - a_ts_recv).microseconds();

      // NB: LMAX Qtys are <QtyA,long>:
...
int main(int argc, char* argv[])
{
  ...
  auto   loggerShP  = IO::MkLogger  ("LMAX_Prod",  logOutput);
  bool   isProd     = true;
  int    debugLevel = pt.get<int>("Main.DebugLevel");

  EPollReactor reactor(loggerShP.get(), debugLevel);

  SecDefsMgr* secDefsMgr = SecDefsMgr::GetPersistInstance(isProd, "LMAX_Prod");

  // The Main Logger (NOT the separate loggers used by Connectors) can be extr-
  // acted from the Reactor and used:

  //-------------------------------------------------------------------------//
  // Instantiate the LMAX FIX Connectors:                                    //
  //-------------------------------------------------------------------------//
  EConnector_FIX_LMAX* mdc = new EConnector_FIX_LMAX(&reactor, secDefsMgr,
    nullptr,  nullptr, GetParamsSubTree(pt, "EConnector_FIX_MDC_LMAX"),
    nullptr); // No PrimaryMDC
  ...
  SecDefD const& eurusd = secDefsMgr->FindSecDef("EUR/USD-LMAX--SPOT");

  //-------------------------------------------------------------------------//
  // Instantiate the Test Strategy:                                          //
  //-------------------------------------------------------------------------//
  TestStrategy_LMAX
    tst("LMAX_Test", &reactor, loggerShP.get(), debugLevel, pt, mdc,
        eurusd);
