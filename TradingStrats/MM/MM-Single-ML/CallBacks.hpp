// vim:ts=2:et
//===========================================================================//
//              "TradingStrats/MM/MM-Single-ML/CallBacks.hpp":               //
...
namespace MAQUETTE {
  ...
  MM_Single_ML<MDC, OMC>::OnTradingEvent
  (
  ...
  )
  {
    ...
    // Log the event:                                                        //
    //-----------------------------------------------------------------------//
    LOG_INFO(1, "MM-Single-ML::OnTradingEvent: {} is now {}",
        a_conn.GetName(),
    ...
    if (!a_on) {
      ...
      if (  // MDCs:
        ...
        if (m_riskMgr != nullptr)
          m_riskMgr->OutputAssetPositions("MM-Single-ML::OnTradingEvent", 0);
        m_reactor->ExitImmediately("MM-Single-ML::OnTradingEvent");
        // This point is not reached!
        __builtin_unreachable();
      }
      else
      if (m_mdc == nullptr || !(m_mdc->IsActive())) {
        // If MDC is inactive, cancell all quotes and wait for reconnect
        CancelAllQuotes(m_mdc, a_ts_exch, a_ts_recv, stratTime);
        LOG_INFO(1, "MM-Single-ML::OnTradingEvent: "
                    "cancelled all quotes, "
                    "waiting for MDC to reconnect")
      } else
      if (m_omc == nullptr || !(m_omc->IsActive())) {
        // If OMC is inactive, just log it and wait,
        // since all quotes have been cancelled automatically
        LOG_INFO(1, "MM-Single-ML::OnTradingEvent: "
                    "waiting for OMC to reconnect")
      }
      ...
      // DelayedGracefulStop hmm(
          // this, stratTime,
          // "MM-Single-ML::OnTradingEvent: Connector Stopped: ", a_conn.GetName());
      // return;
    }
  ...
  template<class MDC, class OMC>
    inline void MM_Single_ML<MDC, OMC>::CheckAllConnectors(
    ...
    for (IPair const& ip : m_iPairs)
      if (!ip.m_OB->IsReady()) {
        ...
        LOG_INFO(
            3,
            "MM-Single-ML::CheckAllConnectors: MDC={}, Instr={}: OrderBook Not "
            "Ready yet",
    ...
    //-----------------------------------------------------------------------//
    if (m_riskMgr != nullptr) {
      LOG_INFO(2, "MM-Single-ML::CheckAllConnectors: Starting the RiskMgr...")
      try {
        //-------------------------------------------------------------------//
        ...
        DelayedGracefulStop hmm(
            this, a_ts_strat,
            "MM-Single-ML::CheckAllConnectors: Cannot Start the RiskMgr: ",
            exc.what());
        return;
      }
    }
    // Produce a log msg:
    LOG_INFO(1,
             "MM-Single-ML::CheckAllConnectors: ALL CONNECTORS ARE NOW ACTIVE{}!",
             (m_riskMgr != nullptr) ? ", RiskMgr Started" : "")
    m_logger->flush();
  }

  //=======================================================================//
  // "OnNewClassPrediction" Call-Back:                                     //
  //=======================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::OnNewClassPrediction
  (
    int            a_class_number,
    utxx::time_val a_ts_exch,
    utxx::time_val a_ts_recv,
    ...
    std::map<std::string, std::pair<unsigned, float*>> const*
                   UNUSED_PARAM(a_features)
  )
  {
    for (IPair& ip: m_iPairs)
    {
      ++ip.m_predCounter;
      if (ip.m_predCounter >= m_nIgnored)
      {
        if (ip.m_ignorePrediction)
          std::cout << "Predictions enabled." << std::endl;
        ip.m_ignorePrediction = false;

        assert(a_class_number <= 1 && a_class_number >= -1);
        ip.m_classPredicted = a_class_number;
        ip.DoQuotes(a_ts_exch, a_ts_recv, utxx::now_utc());
      }
    }
  }
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_ML <MDC, OMC>::OnOrderBookUpdate
  (
  ...
  )
  {
    ...
    if (utxx::unlikely(a_is_error)) {
      ...
      // DelayedGracefulStop hmm(
      //     this, a_ts_strat,
      //     "MM-Single-ML::OnOrderBookUpdate: OrderBook Error: Instr=",
  ...
  //
  template<class MDC, class OMC>
    inline void MM_Single_ML<MDC, OMC>::OnTradeUpdate(Trade const& a_tr) {
  ...
  //=========================================================================//
  template<class MDC, class OMC>
    inline void MM_Single_ML<MDC, OMC>::OnOurTrade(Trade const& a_tr) {
    utxx::time_val stratTime = utxx::now_utc();
    ...
    auto od = GetOrderDetails(req->m_aos, stratTime, "OnOurTrade");
    ...
    int  idx;
    for (idx = 0; idx < ip.m_numOrdersPerSide; ++idx)
      if (req->m_aos == ip.m_AOSes[isBid][idx]) {
        aos = ip.m_AOSes[isBid][idx];
        break;
      }
    ...
    LOG_INFO(2,
             "MM-Single-ML::OnOurTrade: FILL: ReqID={}, AOSID={}, "
             "Instr={}, Side={}, Px={}, ExpPxLast={}, Qty={}, ExecID={}, "
             ": Pos={}",
             req->m_id,
             aos->m_id,
             aos->m_instr->m_FullName.data(),
             isBid ? "Bid" : "Ask",
             double(px),
             double(ori.m_expPx),
             qty,
    ...
    if (m_mktDataLogger != nullptr)
      ...
      LogOurTrade(ip.m_AOSes[isBid][idx]->m_instr->m_FullName.data(),
                  isBid, px, QR(a_tr.GetQty<QT,QR>()), execID);
  }
  ...
  //
  template<class MDC, class OMC>
    inline bool MM_Single_ML<MDC, OMC>::EvalStopConds(utxx::time_val a_ts_exch,
                                       utxx::time_val a_ts_recv,
                                       utxx::time_val a_ts_strat) {
    //----------------------------------------------------------------------//
    ...
    if (utxx::unlikely(m_riskMgr != nullptr &&
      ...
      DelayedGracefulStop hmm(
          this, a_ts_strat,
          "MM-Single-ML::EvalStopConds: RiskMgr entered SafeMode");
      return true;
    }
  ...
  //=========================================================================//
  template<class MDC, class OMC>
    inline void MM_Single_ML<MDC, OMC>::OnCancel(AOS const&     a_aos,
    ...
    if (utxx::unlikely(ori.m_iPairID < 0 ||
      ...
      DelayedGracefulStop hmm(
          this, stratTime, "MM-Single-ML::OnCancel: AOSID=", a_aos.m_id,
    ...
    bool aosFound = false;
    int  idx = 0;
    for (; idx < ip.m_numOrdersPerSide; ++idx)
      if (ip.m_AOSes[isBid][idx] == &a_aos) {
        aosFound = true;
        break;
      }
    ...
    //
    if (utxx::unlikely(!aosFound)) {
      LOG_WARN(
          2,
          "MM-Single-ML::OnCancel: Inconsistency: AOSID={}, Instr={}, {}: "
          "AOS not found.",
          a_aos.m_id,
    ...
    //-----------------------------------------------------------------------//
    LOG_INFO(3,
             "MM-Single-ML::OnCancel: OMC={}, Instr={}, {}, "
             "AOSID={}: CANCELLED",
             a_aos.m_omc->GetName(),
             a_aos.m_instr->m_FullName.data(),
             isBid ? "Bid" : "Ask",
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::OnOrderError
  (
    Req12 const&   a_req,
    ...
    utxx::time_val UNUSED_PARAM(a_ts_recv))
  {
    ...
    if (utxx::unlikely(ori.m_iPairID < 0 ||
      ...
      // Invalid IPairID: This would be a serious error condition:
      DelayedGracefulStop hmm(
          this, stratTime, "MM-Single-ML::OnOrderError: AOSID=", aos->m_id,
    ...
    // Find the AOS index
    // FIXME: "aosFound" is set but not actually used!
    // bool aosFound = false;
    int  idx = 0;
    for (; idx < ip.m_numOrdersPerSide; ++idx)
      if (ip.m_AOSes[isBid][idx] == aos) {
        // aosFound = true;
        break;
      }
    ...
    //
    if (aos->m_isInactive && ip.m_AOSes[isBid][idx] == aos)
      ip.m_AOSes[isBid][idx] = nullptr;
    ...
    LOG_ERROR(
        2,
        "MM-Single-ML::OnOrderError: FAILED: ReqID={}, AOSID={}, Instr={}, "
        "{}: {}{}",
        a_req.m_id,
        aos->m_id,
        aos->m_instr->m_FullName.data(),
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::OnSignal(signalfd_siginfo const& a_si)
  {
    utxx::time_val stratTime = utxx::now_utc();
    ++m_signalsCount;
    ...
    if (m_signalsCount == 1) {
      // Initiate a stop:
      DelayedGracefulStop hmm(this, stratTime, "MM-Single-ML::OnSignal");
    } else {
      ...
      if (m_riskMgr != nullptr)
        m_riskMgr->OutputAssetPositions("MM-Single-ML::OnSignal", 0);
      m_reactor->ExitImmediately("MM-Single-ML::OnSignal");
    }
  }
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::LogOrderBook(
                                      OrderBook const& a_ob,
                                      long             a_lat_ns) const {
    ...
    auto action = [&curr](int, PriceT a_px,
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::LogTrade(Trade const& a_tr) const {
    char* curr = m_mktDataBuff;
    ...
    *curr = ' ';
    ++curr;
    // Trade Qty:
    (void)utxx::ftoa_left(QR(a_tr.GetQty<QT,QR>()), curr, 20, 8);
    *curr = ' ';
    ++curr;
    // Aggressor:
    curr =
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::LogOurTrade(
                                     char const*      a_symbol,
                                     bool             a_is_bid,
                                     PriceT           a_px,
                                     QR               a_qty_lots,
    ...
    *curr = ' ';
    ++curr;
    // Trade Px:
    curr += utxx::ftoa_left(double(a_px), curr, 20, 8);
    *curr = ' ';
    ++curr;
    // Trade Qty (in Lots):
    (void)utxx::ftoa_left(a_qty_lots, curr, 20, 8);
    // Side:
