// vim:ts=2:et
//===========================================================================//
//                   "TradingStrats/MM-Single-ML/InitRun.hpp":                  //
//                              Ctor, Dtor, Run                              //
//===========================================================================//
#pragma once
...
namespace MAQUETTE {
  ...
  inline MM_Single_ML<MDC, OMC>::IPair::IPair() :
      // The Strategy itself:
      m_outer(nullptr),
      // Logging:
      m_logger(nullptr),
      ...
      m_pxStep(NaN<double>),
      m_minSize(NaN<double>),
      m_predCounter(0),
      m_ignorePrediction(true)
  {
    for (int i = 0; i < 2; ++i)
  ...
  inline MM_Single_ML<MDC, OMC>::IPair::IPair(
      ...
      m_pxStep(a_params.get<double>("PxStep", NaN<double>)),
      m_minSize(a_params.get<double>("MinSize", NaN<double>)),
      m_predCounter(0),
      m_ignorePrediction(true) {

    // Check the signs of other params:
    if (utxx::unlikely(m_resistCoeff < 0.0))
      throw utxx::badarg_error("MM-Single-ML::IPair::Ctor: Invalid ResistCoeff=",
                               m_resistCoeff);

    // Verify the Qtys:
    if (utxx::unlikely(m_Qty <= 0 || m_posSoftLimit <= 0))
      throw utxx::badarg_error(
          "MM-Single-ML::IPair::Ctor: Invalid Qty(s) or Limit(s)");

    // Parse VolTargs
    auto targStr  = a_params.get<std::string>("PxVolTargets");
    auto volFracs = a_params.get<std::string>("VolFractions");
    char const* currTarg = targStr.c_str();
    char const* currFrac = volFracs.c_str();
    int i = 0;
    while (true)
    {
      currTarg = utxx::atof(currTarg,
                            targStr.c_str() + targStr.size(),
                            m_volTargs[i]);
      currFrac = utxx::atof(currFrac,
                            volFracs.c_str() + volFracs.size(),
                            m_qtyFracs[i]);
      ++i;
      ...
      if (utxx::unlikely(i > MaxOrdersPerSide))
        throw utxx::badarg_error(
          "MM-Single-ML::IPair::Ctor: Too many orders per side");
    }
    m_numOrdersPerSide = i;
    assert(m_numOrdersPerSide > 0);
  }
  ...
  //
  template<class MDC, class OMC>
  inline bool MM_Single_ML<MDC, OMC>::IPair::operator==(MM_Single_ML::IPair
      const& right) const {
  ...
  //=========================================================================//
  // "MM_Single_ML::Get{MDC,OMC}":                                       //
  //=========================================================================//
  // These methods either throw an exception, or return a non-NULL ptr:
  //
  template<class MDC, class OMC>
  inline EConnector_MktData* MM_Single_ML<MDC, OMC>::GetMDC() const
    { return m_mdc; }

  template<class MDC, class OMC>
  inline EConnector_OrdMgmt* MM_Single_ML<MDC, OMC>::GetOMC() const
    { return m_omc; }

  //=========================================================================//
  // "MM_Single_ML": Non-Default Ctor: //
  //=========================================================================//
  template<class MDC, class OMC>
  inline MM_Single_ML<MDC, OMC>::MM_Single_ML(
      ...
      m_iPairs(),
      m_fixedRates(),
      m_nIgnored(a_params.get<int>("Main.IgnorePeriods", 0))
  {
    //-----------------------------------------------------------------------//
    ...
    if (utxx::unlikely(m_reactor == nullptr || m_logger == nullptr))
      throw utxx::badarg_error(
          "MM-Single-ML::Ctor: Reactor and Logger must be non-NULL");
    ...
    for (auto& sect : a_params)
      if (strncmp(sect.first.data(), "Pair", 4) == 0) {
        ...
        LOG_INFO(2, "MM-Single-ML::Ctor: Created MDC: {}", MDC::Name)
        m_logger->flush();
        ...
        LOG_INFO(2, "MM-Single-ML::Ctor: Created OMC: {}", OMC::Name)
        m_logger->flush();
        ...
        if (utxx::unlikely(prev != m_iPairs.cend()))
          throw utxx::badarg_error(
              "MM-Single-ML::Ctor: Duplicate Pair", nPairs + 1,
        ...
        if (utxx::unlikely(nPairs >= MaxPairs)) {
          ...
          throw utxx::badarg_error(
              "MM-Single-ML::Ctor: Too Many Pairs: MaxAllowed=", maxPairs);
        }
        // If OK:
    ...
    if (utxx::unlikely(m_mktDataDepth > 50))
      throw utxx::badarg_error("MM-Single-ML Ctor:: Too large MktDataDepth: ",
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::Run() {
  ...
  //
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::SemiGracefulStop() {
    ...
    if (m_riskMgr != nullptr)
      m_riskMgr->OutputAssetPositions("MM-Single-ML::SemiGracefulStop", 0);
  ...
  //-------------------------------------------------------------------------//
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::DelayedGracefulStop::Out()
  { m_msg << ": EXITING..."; }

  template<class MDC, class OMC>
  template <typename T, typename... Args>
  inline void MM_Single_ML<MDC, OMC>::DelayedGracefulStop::Out(T const& a_t,
                                                  Args&&... a_args) {
    m_msg << a_t;
  ...
  inline MM_Single_ML<MDC, OMC>::DelayedGracefulStop::DelayedGracefulStop(
      MM_Single_ML<MDC, OMC>* a_outer,
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::CancelAllQuotes(
    ...
    for (IPair& ip : m_iPairs)
      if (a_mdc == nullptr || a_mdc == ip.m_MDC) {
        ...
        for (int i = 0; i < ip.m_numOrdersPerSide; ++i) {
