//                   "TradingStrats/MM-Single-ML/Orders.hpp":               //
...
namespace MAQUETTE {
  ...
  template <class MDC, class OMC>
    inline void MM_Single_ML<MDC, OMC>::IPair::CancelQuotes(
                                             utxx::time_val a_ts_exch,
                                             utxx::time_val a_ts_recv,
                                             utxx::time_val a_ts_strat) {
    for (int s = 0; s < 2; ++s)
      for (int i = 0; i < m_numOrdersPerSide; ++i)
  ...
  //=========================================================================//
  template <class MDC, class OMC>
    inline void MM_Single_ML<MDC, OMC>::IPair::DoQuotes(
                                         utxx::time_val a_ts_exch,
                                         utxx::time_val a_ts_recv,
                                         utxx::time_val a_ts_strat) {
    //-----------------------------------------------------------------------//
    ...
    double pxStep  = (m_pxStep > 0.0)
                     ? m_pxStep
                     : m_Instr->m_PxStep;
    QR     minSize = (m_minSize > 0.0)
                     ? m_minSize
                     : m_Instr->m_LotSize * QR(m_Instr->m_MinSizeLots);
    ...
    if (utxx::unlikely(!(IsFinite(bestBidPx) && IsFinite(bestAskPx)))) {
      ...
      LOG_WARN(
          2,
          "MM-Single-ML::IPair::DoQuotes: PassInstr={}: "
    ...
    // NB: We reserve space for MAX, but use only up to m_numOrdersPerSide:
    QR     sideQtys[2]                  = {{0.0}};
    ...
    for (int s = 0; s < 2; ++s) {
      ...
      for (auto i = 0; i < m_numOrdersPerSide; ++i) {

        // Class prediction
        // Cancel orders in the direction of prediction
        if ( !m_ignorePrediction && (
                                      (isBid && m_classPredicted == -1) ||
                                      (!isBid && m_classPredicted == 1)
                                    )
           )
        {
          // Set px to 0. If AOS is not empty, it will be cancelled later
          px[i] = PriceT();
        }
        ...
        // So: is the price really unchanged (either because it was such by
        // calculation, or forced do stay put)? NB: If "px" was not Finite at
        // all, the corresp "unchPxs" flag remains "false", because the equality
        // fails:
      ...
      if (IsFinite(newQPxs[Ask][i]) && IsFinite(newQPxs[Bid][i]))
      {
        ...
        if (IsFinite(newQPxs[Ask][i]) && IsFinite(newQPxs[Bid][i])
          ...
          if (bpsResSpread < m_bpsSpreadThresh) {
            LOG_WARN(4,
                     "MM-Single-ML::IPair::DoQuotes: Instr={}: "
                     "Below spread thresh.",
                     m_Instr->m_Symbol.data())
    ...
    // LOG_INFO(4, "MM-Single-ML::IPair::DoQuotes: "
    ...
    if (utxx::unlikely(m_debugLevel >= 2 && doneSomething)) {
      ...
      m_logger->info("MM-Single-ML::IPair::DoQuotes: Instr={}"
                     "\nQuoted: [{}]{}@{} .. {}@{}[{}]"
                     "\nBest:          {} .. {}"
                     "\nPosition:      {}",
  ...
  //=========================================================================//
  // Distributes the qtys among "m_numOrdersPerSide: of orders per side
  //
  template <class MDC, class OMC>
  inline void MM_Single_ML<MDC, OMC>::IPair::DistributeQtys(
                                                QR const      a_targ_qty,
                                                ...
                                                QR*           a_qtys) const
  {
    if (utxx::unlikely(a_targ_qty < a_min_size)) {
      LOG_INFO(4, "MM_Single_ML::IPair::DistributeQtys: "
    ...
    if (a_qtys[m_numOrdersPerSide] > 0)
      LOG_WARN(4, "MM_Single_ML::IPair::DistributeQtys: "
          "Leftover qty {}", a_qtys[m_numOrdersPerSide])
    return;
  }
  ...
  inline void
  MM_Single_ML<MDC, OMC>::IPair::GetPxsAtVols(
      bool    a_is_bid,
      PriceT* a_prices) const
  {
    // Traverse the corresp side of the Passive OrderBook, up to the total size
    QR resVols = 0;
    double pxStep = (m_pxStep > 0.0) ? m_pxStep : m_Instr->m_PxStep;
    int currLevel = 0;
    bool isTopLevel = true;

    auto scanner = [this,       &a_is_bid, &resVols,
                    &currLevel, &a_prices, &isTopLevel]
      (int, PriceT a_px_level, OrderBook::OBEntry const& a_obe) -> bool
    {
      if (currLevel >= this->m_numOrdersPerSide)
        // Covered all orders
        return false;
      resVols += QR(a_obe.GetAggrQty<QT,QR>());
      for (int tmp = currLevel; tmp < this->m_numOrdersPerSide; ++tmp)
      {
    ...
    // The result:
    if (ReturnPrevious)
      for (auto i = 0; i < this->m_numOrdersPerSide; ++i)
  ...
  // rivals to MM-Single-ML. Returns the corresp size (in Base Units) of the Aggr
  // Instr:
  // XXX: This computation is relatively costly!
  //
  template <class MDC, class OMC>
  template <bool StopAtOurs>
  inline typename MM_Single_ML<MDC, OMC>::QR
  MM_Single_ML<MDC, OMC>::IPair::GetOthersVolsUpTo(
      bool   a_is_bid,
  ...
  inline AOS const* MM_Single_ML<MDC, OMC>::IPair::NewOrderSafe
  (
    bool            a_is_bid,
    OrderID         a_ref_aosid,
    PriceT          a_px1,
  ...
  )
  {
    ...
    if (utxx::unlikely(m_outer->m_riskMgr != nullptr &&
      ...
      MM_Single_ML<MDC, OMC>::DelayedGracefulStop hmm(
          m_outer, a_ts_strat,
          "MM-Single-ML::IPair::NewOrderSafe: RiskMgr has entered SafeMode");
      return nullptr;
    }
    ...
    try {
      ...
      MM_Single_ML<MDC, OMC>::DelayedGracefulStop hmm(
          m_outer, a_ts_strat,
          "MM-Single-ML::IPair::NewOrderSafe: Order FAILED: Instr=",
    ...
    if (utxx::unlikely(res != nullptr)) {
      ...
      LOG_INFO(
          3,
          "MM-Single-ML::IPair::NewOrderSafe: OMC={}, Instr={}, NewAOSID={}, "
          "NewReqID={}, {}, Qty={}, Px={}",
          res->m_omc->GetName(),
  ...
  template <class MDC, class OMC>
  template <bool Buffered>
  inline void MM_Single_ML<MDC, OMC>::IPair::CancelOrderSafe(
    ...
    try {
      ...
      DelayedGracefulStop hmm(
          m_outer, a_ts_strat,
          "MM-Single-ML::IPair::CancelOrderSafe: FAILED: Instr=",
    ...
    LOG_INFO(
        3,
        "MM-Single-ML::IPair::CancelOrderSafe: OMC={}, Instr={}, {}: "
  ...
  template <class MDC, class OMC>
  template <bool Buffered>
  inline bool MM_Single_ML<MDC, OMC>::IPair::ModifyQuoteSafe(
      bool           a_is_bid,
      int            a_idx,
    ...
    if (utxx::unlikely(aos == nullptr || aos->m_isInactive ||
      ...
      LOG_WARN(2, "MM-Single-ML::ModifyQuoteSafe: Invalid AOSID={},"
               "Last ReqID={}, "
               "{}{}{}",
    ...
    try {
      ...
      LOG_WARN(
          2,
          "MM-Single-ML::IPair::ModifyQuoteSafe: FAILED: Instr={}, {}, Px={}, "
    ...
    LOG_INFO(
        3,
        "MM-Single-ML::IPair::ModifyQuoteSafe: OMC={}, Instr={}, {}, AOSID={}, "
        "NewReqID={}, Qty={}, Px={}",
        aos->m_omc->GetName(),
        instr.m_FullName.data(),
        aos->m_isBuy ? "Bid" : "Ask",
  ...
  //=========================================================================//
  template <class MDC, class OMC>
  inline std::pair<typename MM_Single_ML<MDC, OMC>::OrderInfo*,
                   typename MM_Single_ML<MDC, OMC>::IPair*>
  MM_Single_ML<MDC, OMC>::GetOrderDetails(AOS const*     a_aos,
                             utxx::time_val a_ts_strat,
                             char const*    a_where) {
    ...
    if (utxx::unlikely(ori.m_iPairID < 0 ||
      ...
      DelayedGracefulStop hmm(this, a_ts_strat, "MM-Single-ML::GetOrderDetails(",
                              a_where, "): AOSID=", a_aos->m_id,
                              ", Instr=", a_aos->m_instr->m_FullName.data(),
                              (a_aos->m_isBuy ? ", Bid" : ", Ask"),
                              ": Invalid IPairID=", ori.m_iPairID);

      return std::make_pair<MM_Single_ML<MDC, OMC>::OrderInfo*,
                            MM_Single_ML<MDC, OMC>::IPair*>(nullptr, nullptr);
    }
    // If OK:
