// vim:ts=2:et
//===========================================================================//
//              "TradingStrats/MM/MM-Single-Net/CallBacks.hpp":              //
...
namespace MAQUETTE {
  ...
  MM_Single_Net<MDC, OMC>::OnTradingEvent(EConnector const& a_conn,
                            bool              a_on,
                            SecID             a_sec_id,
    ...
    // Log the event:                                                        //
    //-----------------------------------------------------------------------//
    LOG_INFO(1, "MM-Single-Net::OnTradingEvent: {} is now {}",
        a_conn.GetName(),
    ...
    if (!a_on) {
      ...
      if (  // MDCs:
        ...
        if (m_riskMgr != nullptr)
          m_riskMgr->OutputAssetPositions("MM-Single-Net::OnTradingEvent", 0);
        m_reactor->ExitImmediately("MM-Single-Net::OnTradingEvent");
      ...
      if (m_mdc == nullptr || !(m_mdc->IsActive())) {
        ...
        LOG_INFO(1, "MM-Single-Net::OnTradingEvent: "
      ...
      // DelayedGracefulStop hmm(
          // this, stratTime,
          // "MM-Single-Net::OnTradingEvent: Connector Stopped: ", a_conn.GetName());
      // return;
    }
  ...
  template<class MDC, class OMC>
    inline void MM_Single_Net<MDC, OMC>::CheckAllConnectors(
    ...
    for (IPair const& ip : m_iPairs)
      if (!ip.m_OB->IsReady()) {
        ...
        LOG_INFO(
            3,
            "MM-Single-Net::CheckAllConnectors: MDC={}, Instr={}: OrderBook Not "
            "Ready yet",
    ...
    //-----------------------------------------------------------------------//
    if (m_riskMgr != nullptr) {
      LOG_INFO(2, "MM-Single-Net::CheckAllConnectors: Starting the RiskMgr...")
      try {
        //-------------------------------------------------------------------//
        ...
        DelayedGracefulStop hmm(
            this, a_ts_strat,
            "MM-Single-Net::CheckAllConnectors: Cannot Start the RiskMgr: ",
            exc.what());
        return;
      }
    }
    // Produce a log msg:
    LOG_INFO(1,
             "MM-Single-Net::CheckAllConnectors: ALL CONNECTORS ARE NOW ACTIVE{}!",
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_Net<MDC, OMC>::OnOrderBookUpdate
  (
  ...
  )
  {
    ...
    if (utxx::unlikely(a_is_error)) {
      ...
      // DelayedGracefulStop hmm(
      //     this, a_ts_strat,
      //     "MM-Single-Net::OnOrderBookUpdate: OrderBook Error: Instr=",
  ...
  //
  template<class MDC, class OMC>
    inline void MM_Single_Net<MDC, OMC>::OnTradeUpdate(Trade const& a_tr) {
  ...
  //=========================================================================//
  template<class MDC, class OMC>
    inline void MM_Single_Net<MDC, OMC>::OnOurTrade(Trade const& a_tr) {
    utxx::time_val stratTime = utxx::now_utc();
    ...
    int  idx;
    for (idx = 0; idx < ip.m_numOrders; ++idx)
    ...
    LOG_INFO(2,
             "MM-Single-Net::OnOurTrade: FILL: ReqID={}, AOSID={}, "
  ...
  //
  template<class MDC, class OMC>
    inline bool MM_Single_Net<MDC, OMC>::EvalStopConds(utxx::time_val a_ts_exch,
                                       utxx::time_val a_ts_recv,
                                       utxx::time_val a_ts_strat) {
    //----------------------------------------------------------------------//
    ...
    if (utxx::unlikely(m_riskMgr != nullptr &&
      ...
      DelayedGracefulStop hmm(
          this, a_ts_strat,
          "MM-Single-Net::EvalStopConds: RiskMgr entered SafeMode");
      return true;
    }
  ...
  //=========================================================================//
  template<class MDC, class OMC>
    inline void MM_Single_Net<MDC, OMC>::OnCancel(AOS const&     a_aos,
    ...
    if (utxx::unlikely(ori.m_iPairID < 0 ||
      ...
      DelayedGracefulStop hmm(
          this, stratTime, "MM-Single-Net::OnCancel: AOSID=", a_aos.m_id,
    ...
    if (utxx::unlikely(!aosFound)) {
      LOG_WARN(
          2,
          "MM-Single-Net::OnCancel: Inconsistency: AOSID={}, Instr={}, {}: "
          "AOS not found.",
          a_aos.m_id,
    ...
    //-----------------------------------------------------------------------//
    LOG_INFO(3,
             "MM-Single-Net::OnCancel: OMC={}, Instr={}, {}, "
             "AOSID={}: CANCELLED",
             a_aos.m_omc->GetName(),
             a_aos.m_instr->m_FullName.data(),
             isBid ? "Bid" : "Ask",
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_Net<MDC, OMC>::OnOrderError
  (
    Req12 const&   a_req,
    ...
    utxx::time_val UNUSED_PARAM(a_ts_recv))
  {
    ...
    if (utxx::unlikely(ori.m_iPairID < 0 ||
      ...
      // Invalid IPairID: This would be a serious error condition:
      DelayedGracefulStop hmm(
          this, stratTime, "MM-Single-Net::OnOrderError: AOSID=", aos->m_id,
    ...
    LOG_ERROR(
        2,
        "MM-Single-Net::OnOrderError: FAILED: ReqID={}, AOSID={}, Instr={}, "
        "{}: {}{}",
        a_req.m_id,
        aos->m_id,
        aos->m_instr->m_FullName.data(),
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_Net<MDC, OMC>::OnSignal(signalfd_siginfo const& a_si)
  {
    utxx::time_val stratTime = utxx::now_utc();
    ++m_signalsCount;
    ...
    if (m_signalsCount == 1) {
      // Initiate a stop:
      DelayedGracefulStop hmm(this, stratTime, "MM-Single-Net::OnSignal");
    } else {
      ...
      if (m_riskMgr != nullptr)
        m_riskMgr->OutputAssetPositions("MM-Single-Net::OnSignal", 0);
      m_reactor->ExitImmediately("MM-Single-Net::OnSignal");
    }
  }
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_Net<MDC, OMC>::LogOrderBook(
                                      OrderBook const& a_ob,
                                      long             a_lat_ns) const {
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_Net<MDC, OMC>::LogTrade(Trade const& a_tr) const {
    char* curr = m_mktDataBuff;
  ...
  //=========================================================================//
  template<class MDC, class OMC>
  inline void MM_Single_Net<MDC, OMC>::LogOurTrade(
                                     char const*      a_symbol,
                                     bool             a_is_bid,
                                     PriceT           a_px,
                                     QR               a_qty_lots,
